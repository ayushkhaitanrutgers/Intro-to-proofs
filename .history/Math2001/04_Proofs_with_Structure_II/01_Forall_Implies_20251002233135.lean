...
example {a b : ℝ} (h : ∀ x, x ≥ a ∨ x ≤ b) : a ≤ b := by
  ...
  have hc : a/2 ≤ b/2 := by
    calc
      b/2 = b-b/2 := by ring
      _ ≥ (a+b)/2 - b/2 := by rel[hb]
      _ = a/2 + b/2 - b/2 := by ring
      _ = a/2 := by ring
  calc
    a = a/2 + a/2 := by ring
    _ ≤ a/2 + b/2 := by rel[hc]
...
example : ∃ c : ℝ, ∀ x y, x ^ 2 + y ^ 2 ≤ 4 → x + y ≥ c := by
  ...
  have h1 : (x+y)^2 ≤ 3^2 := by
    calc
      ...
      _ ≤ 9 := by numbers
      _ = 3^2 := by numbers
  have hc : 0 ≤ 3 := by numbers
  have h3 := abs_le_of_sq_le_sq' h1 hc
  obtain ⟨ ha, hb⟩ := h3
  exact ha
...
example : Prime 2 := by
  ...
  intro m hm
  have hp : 0 < 2 := by numbers
  have h1 : m ≤ 2 := Nat.le_of_dvd hp hm
  have h2 : m ≥ 1 := Nat.pos_of_dvd_of_pos hm hp
  interval_cases m
  left
  rfl
  right
...
example {n : ℤ} (hn : ∀ m, 1 ≤ m → m ≤ 5 → m ∣ n) : 15 ∣ n := by
  have ha : 1 ≤ 3 := by numbers
  have hb : 3 ≤ 5 := by numbers
  have hc : 3 ∣ n := by apply hn ha hb
  have hd : 1 ≤ 5 := by numbers
  have he : 5 ≤ 5 := by numbers
  have hf : 5 ∣ n := by apply hn hd he
  dsimp[(· ∣ ·)] at *
  sorry
