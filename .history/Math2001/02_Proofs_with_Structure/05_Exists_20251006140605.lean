...
example {t : ℝ} (h : ∃ a : ℝ, a * t + 1 < a + t) : t ≠ 1 := by
  ...
  --there has to be ∃ in the goal for you to be able to use the "use" tactic
...
-- what does this question mean? how can there be an exists sign in the hypothesis, but not in the goal?
-- consider the following example: suppose your question is: hypothesis- there exists a number whose square is equal to 4. goal: prove that number is not equal to 1.
-- the way to prove it is to show that that number is +2 or -2, and hence cannot be equal to 1.
