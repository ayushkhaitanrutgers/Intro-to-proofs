...
example (a b : ℤ) : (a + b) ^ 3 ≡ a ^ 3 + b ^ 3 [ZMOD 3] :=
  calc
    (a+b)^3
      = a^3+b^3 + 3*a*a*b + 3*a*b*b := by ring
    _ ≡ a^3+b^3+3*(a^2*b+a*b^2) := 
    _ ≡ a^3 + b^3  [ZMOD 3] := by sorry

example : ∃ a : ℤ, 4 * a ≡ 1 [ZMOD 7] := by
  use 2
  calc
    4*2
      ≡ 1+7*1 [ZMOD 7] := by numbers
    _ ≡ 1 [ZMOD 7] := by extra
...
example : ∃ k : ℤ, 5 * k ≡ 6 [ZMOD 8] := by
  ...
  calc
    5*6
      ≡ 30 [ZMOD 8] := by numbers
    _ = 6 + 8*3 := by numbers
    _ ≡ 6 + 8*3 [ZMOD 8] := by extra
    _ ≡ 6 [ZMOD 8] := by extra
example (n : ℤ) : 5 * n ^ 2 + 3 * n + 7 ≡ 1 [ZMOD 2] := by
  mod_cases n%2
  calc
    5 * n ^ 2 + 3 * n + 7
      ≡ 5*0^2 + 3*0 + 7 [ZMOD 2] := by rel[H]
    _ =1+2*3 := by numbers
    _ ≡ 1 [ZMOD 2] := by extra
  calc
    5 * n ^ 2 + 3 * n + 7
      ≡ 5*1^2 + 3*1 + 7 [ZMOD 2] := by rel[H]
    _ =1+2*7 := by numbers
    _ ≡ 1 [ZMOD 2] := by extra
...
example {x : ℤ} : x ^ 5 ≡ x [ZMOD 5] := by
  ...
  calc
    ...
    _  = 0 := by ring
    _ ≡ x [ZMOD 5] := by rel[H]
  sorry --the same thing repeated in all the steps below
  sorry
  sorry
  sorry
